...
type Panel = 'story' | 'wipepaper' | 'buy' | 'chart' | 'contact' | 'bored' | null;
...
export default function Page() {
  ...
  // Lock single-screen layout (no page scroll)
  useEffect(() => {
  ...
  return (
    <main className="screen">
      ...
      {/* Header */}
      <header className="topbar" onClick={() => menuOpen && setMenuOpen(false)}>
        {/* brand was removed intentionally (logo is in bg image) */}
        ...
        >
          ...
          {/* NEW: Bored? opens as overlay panel (not a route) */}
          <button
            className={`btn navbtn ${open === 'bored' ? 'active' : ''}`}
            onClick={() => {
              setOpen('bored');
              setMenuOpen(false);
            }}
      ...
      <section className="center">
        ...
        <p className="tag">
          Born mid-poop by Satoshi Flushimoto. 1,000,000,000 supply. Utility: none. Lore:
          everything.
        </p>
...
function Story() {
  return (
    <div className="copy">
      <p>
        Past midnight on a dead-still highway, Satoshi Flushimoto—more liquidations than
        wins—hunted for relief when destiny (and indigestion) struck.
      </p>
      <p>
        He found a forgotten gas station, a grimy stall, and—mid-poop, straining—the
        revelation: if the market is a toilet, it needs a Final Flush. He deployed{' '}
        <strong>TOILETCOIN</strong> on Pump.fun, minted <strong>1,000,000,000</strong>,
        bought the first <strong>1,000,000</strong>, scrawled the contract on a tile, wrote
        the Wipepaper on two-ply, and vanished.
      </p>
      <p>
        Days later a janitor posted the artifacts. The owner built a shrine. Pilgrims now
        queue for the next stall. Profits? ~<strong>6,900,000×</strong>.
      </p>
    </div>
  );
}
      ...
      <form onSubmit={handleSubmit} className="form" style={{ maxWidth: 520 }}>
        ...
        <div className="field">
          ...
          <textarea
            ...
            name="message"
            required
            placeholder="Say hi, propose chaos, request a shrine…"
            rows={6}
          />
        </div>
        <div className="form-row">
          ...
          <a className="btn" href="mailto:contact@toiletcoin.wtf">
            Or email directly
          </a>
        </div>
...
/** BORED? — Overlay panel (placeholder for the mini-game) */
function BoredPanel() {
  return (
    <div className="copy" style={{ textAlign: 'center' }}>
      <h3 style={{ marginTop: 0 }}>Bored?</h3>
      <p>
        Mini-game coming soon. Mission: catch &amp; flush as many shitcoins as possible to
        clean the chain. Lose, and the Pump.fun overflow warning appears. 😅
      </p>
      <p style={{ opacity: 0.85 }}>
        We’ll ship a pixel-art toilet + falling coins, water plop SFX, and score/combo
        counters. Keyboard + touch support.
      </p>
    </div>
  );
}
